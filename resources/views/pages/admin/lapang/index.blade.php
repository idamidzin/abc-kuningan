...
<div class="container-fluid">
	...
	<div class="card">
		<div class="card-header">
			<div class="card-title">
				<a href="{{ route('admin.lapang.index') }}" class="{{ $is_trash ? 'text-success':'text-muted' }}"> Semua
					<span class="badge badge-pill badge-info">{{ $lapang_count }}</span>
				</a>
				&nbsp; | &nbsp;
				<a href="{{ route('admin.lapang.index') }}?status=trash" class="{{ $is_trash ? 'text-muted':'text-danger' }}">
					Sampah
					<span class="badge badge-pill badge-danger">{{ $trash_count }}</span>
				</a>
				<div class="float-right">
					<a href="{{ route('admin.lapang.create') }}" class="btn btn-purple">Data Baru</a>
				</div>
			</div>
		</div>
		<div class="card-body">
					...
					<tbody>
						...
						<tr>
							...
							<td class="text-center">
								<img src="{{ asset('storage/lapang/'.$row->foto) }}" width="100px">
							</td>
							<td class="text-center" width="100px">
								...
								<a href="{{ route('admin.lapang.edit', $row->hashid) }}" class="btn btn-success btn-sm">
	...
	<script type="text/javascript">
		...
		function execRemove(method, hashid) {
			$("#action-form").attr('action', 'lapang/delete/' + hashid);
		...
		function restore (hashid) {
			$("#action-form").attr('action', 'lapang/restore/' + hashid);
